...
MODEL_CLASSES = {
    ...
    # "gpt": (GPTConfig, GPTForCausalLMAuto, None),
}
...
# pp 动手训练的参数名 => 动半参数名
def map_structure_name(hand_name):
    fs = hand_name.split(".")
    if "word_embeddings" in hand_name:
        result = "gpt.embeddings.word_embeddings.weight"
    elif "position_embeddings" in hand_name:
        result = "gpt.embeddings.position_embeddings.weight"
    elif "shared_layers" in hand_name:
        result = "lm_head.weight"
    else:
        idx = int(fs[1])
        if idx == 25:
            result = f"gpt.decoder.norm." + ".".join(fs[2:])
        else:
            result = f"gpt.decoder.layers.{idx-1}." + ".".join(fs[2:])
    print(f"[YAMEI0220] map_structure_name =>{hand_name}, result=>{result}")
    return result


# LOAD_FILE = "hand_single"
# LOAD_FILE = "hand_dp2mp2"
LOAD_FILE = "hand_dp2mp2pp2"


def load_model_auto_yamei(model, dtype):
    print(f"========load_model_auto_yamei start=======")
    # check pretrained param
    pretrained_state_dict = paddle.load(f"{LOAD_FILE}/all.pdparams")
    model_state_dict = model.state_dict()
    assert len(pretrained_state_dict) >= len(
        model_state_dict
    ), f"{len(pretrained_state_dict)} vs {len(model_state_dict)}"
    print(f"pretrained_state_dict count {len(pretrained_state_dict)} model_state_dict count {len(model_state_dict)}")

    print(f" -------- 0. model_state_dict start--------")
    for (k, v) in model_state_dict.items():
        print(f"{k}=>{v.shape}=>md5:{v._md5sum()}")
    print(f" -------- 0. model_state_dict end--------")

    # print(f" -------- 1. pretrained_state_dict start--------")
    # for (k, v) in pretrained_state_dict.items():
    #     print(f"{k}=>{v.shape}=>md5:{v._md5sum()}")
    # print(f" -------- 1. pretrained_state_dict end--------")

    new_state_dict = OrderedDict()
    for (k, v) in pretrained_state_dict.items():
        k = map_structure_name(k)
        if dtype == "float16":
            v = paddle.cast(v, "float16")
        elif dtype == "bfloat16":
            v = paddle.cast(v, "bfloat16")
        new_state_dict[k] = v
    assert len(new_state_dict) == len(model_state_dict), f"{len(new_state_dict)} vs {len(model_state_dict)}"

    # print(f" -------- 2. new_state_dict start--------")
    for (k, v) in new_state_dict.items():
        assert k in model_state_dict, f"{k} not in model_state_dict"
        print(f"{k}=>{v.shape}=>md5:{v._md5sum()}")
    # print(f" -------- 2. new_state_dict end--------")
    model.set_state_dict(new_state_dict)
    print(f"========load_model_auto_yamei end=======")


def load_model_auto_yamei_2(model):
    print(f"========load_model_auto_yamei start=======")
    # check pretrained param
    pretrained_state_dict = paddle.load(f"{LOAD_FILE}/all.pdparams")
    model_state_dict = model.state_dict()
...
# os.environ["TRANSLATOR_CODE_LEVEL"] = '100'
...
class ModelArguments:
    ...
    # 将原本的 0.1 修改为 0
    hidden_dropout_prob: float = field(default=0, metadata={"help": "The hidden dropout prob."})
    attention_probs_dropout_prob: float = field(default=0, metadata={"help": "The attention hidden dropout prob."})
...
def main():
    ...
    load_model_auto_yamei(model, dtype)
...
def shard_model(model):
    ...
    for name, layer in model.named_sublayers(include_self=False):
        ...
        if "word_embeddings" in name or "position_embeddings" in name:
            # NOTE: embedding only support column split now. it will update in the future
